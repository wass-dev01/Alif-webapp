...
(() => {
  ...
  function toggleTheme() {
    ...
    try { localStorage.setItem('alif_theme', isLight ? 'light' : 'dark'); } catch {}
    // Update icon: sun when dark (click to go light), moon when light (click to go dark)
  ...
  function loadTheme() {
    try {
      const saved = localStorage.getItem('alif_theme');
  ...
  async function sendTestNotification() {
    ...
    if (Notification.permission !== 'granted') {
      const perm = await Notification.requestPermission();
      if (perm !== 'granted') {
        alert('Les notifications ne sont pas autorisées.\n\nSur iPhone : ajoutez d\'abord l\'app à l\'écran d\'accueil, puis autorisez les notifications dans Réglages.');
        return;
      }
      checkNotifBanner();
    }
    ...
    setTimeout(() => {
      try {
        new Notification('🕌 Test Alif — أذان', {
